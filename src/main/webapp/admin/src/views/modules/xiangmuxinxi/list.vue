<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">项目编号</label>
						<el-input v-model="searchForm.xiangmubianhao" placeholder="项目编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">项目名称</label>
						<el-input v-model="searchForm.xiangmumingcheng" placeholder="项目名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">项目地点</label>
						<el-input v-model="searchForm.xiangmudidian" placeholder="项目地点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="项目进度" prop="xiangmujindu">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">项目进度</label>
						<el-select clearable v-model="searchForm.xiangmujindu" placeholder="请选择项目进度" >
							<el-option v-for="(item,index) in xiangmujinduOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('xiangmuxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('xiangmuxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<download-excel v-if="isAuth('xiangmuxinxi','导出')" class="export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "项目信息.xls">
						<!-- 导出excel -->
					...
					<el-button class="btn18" v-if="isAuth('xiangmuxinxi','项目统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="xiangmuchengben"
						label="项目成本">
						<template slot-scope="scope">
							{{scope.row.xiangmuchengben}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="xiangmujindu"
						label="项目进度">
						<template slot-scope="scope">
							{{scope.row.xiangmujindu}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xiangmuxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('xiangmuxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xiangmuxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="xiangmujinduChart1" style="width:100%;height:600px;"></div>
...
//导出excel
				json_fields: {
					"项目编号": "xiangmubianhao",    //常规字段
					"项目名称": "xiangmumingcheng",    //常规字段
					"项目地点": "xiangmudidian",    //常规字段
					"项目规模": "xiangmuguimo",    //常规字段
					"项目负责人": "xiangmufuzeren",    //常规字段
					"开始时间": "kaishishijian",    //常规字段
					"结束时间": "jieshushijian",    //常规字段
					"项目成本": "xiangmuchengben",    //常规字段
					"项目进度": "xiangmujindu",    //常规字段
				},
				json_meta: [
					[
						{
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var xiangmujinduChart1 = echarts.init(document.getElementById("xiangmujinduChart1"),'macarons');
        this.$http({
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.xiangmudidian!='' && this.searchForm.xiangmudidian!=undefined){
            params['xiangmudidian'] = '%' + this.searchForm.xiangmudidian + '%'
          }
           if(this.searchForm.xiangmujindu!='' && this.searchForm.xiangmujindu!=undefined){
            params['xiangmujindu'] = this.searchForm.xiangmujindu
          }
	...
	xiangmuxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'xiangmuxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
